...
from sklearn.metrics import matthews_corrcoef, f1_score, recall_score, precision_score, roc_auc_score
...
def compute_metrics(task_name, preds, labels):
    ...
    if task_name == "cola":
        ...
        return tip_report(preds, labels)
    else:
        raise KeyError(task_name)

def compute_auc(ori_preds, labels):
    assert(len(ori_preds) == len(labels))
    # fetch preds for label 1
    target_preds = []
    for i in range(len(ori_preds)):
        target_preds.append(ori_preds[i][1])
    return roc_auc_score(labels, target_preds)
